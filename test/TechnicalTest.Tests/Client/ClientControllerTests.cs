...
public class ClientControllerTests
{
    ...
    public async Task GetClientsAsync_ShouldReturnOkWithClients()
    {
        ...
        {
            new(Guid.NewGuid(), Guid.NewGuid(), "Alice", "Smith", "Madrid", 100m, "email", DateTime.UtcNow),
            new(Guid.NewGuid(), Guid.NewGuid(), "Bob", "Jones", "Barcelona", 200m, "sms", DateTime.UtcNow)
        };
        var cts = new CancellationTokenSource();
        _clientService
    ...
    public async Task GetClientByIdAsync_ShouldReturnOkWithClient()
    {
        ...
        var expected = new ClientDto(clientId, Guid.NewGuid(), "Alice", "Smith", "Madrid", 100m, "email", DateTime.UtcNow);
        var cts = new CancellationTokenSource();
        _clientService
    ...
    {
        var sut = CreateSut();
        var request = new ClientCreateRequestDto(Guid.NewGuid(), "Alice", "Smith", "Madrid", 100m, "email");
        var created = new ClientDto(Guid.NewGuid(), request.UserId, request.FirstName, request.LastName, request.City, request.Balance ?? 0m, request.NotificationChannel ?? "email", DateTime.UtcNow);
        var cts = new CancellationTokenSource();
        _clientService
    ...
    public async Task UpdateClientAsync_ShouldReturnUpdatedClient()
    {
        ...
        var request = new ClientUpdateRequestDto("Alice", "Smith", "Madrid", 150m, "sms", userId);
        var updated = new ClientDto(clientId, userId, request.FirstName, request.LastName, request.City, request.Balance ?? 0m, request.NotificationChannel ?? "email", DateTime.UtcNow);
        var cts = new CancellationTokenSource();
        _clientService
